...
if isfield(p,'xmax'               ),xmax                = p.xmax ;
else
    xmax               = pi     ;
end
if isfield(p,'xmin'               ),xmin                = p.xmin     ;
else
    xmin               = -pi     ;
end
...
if online_printing==2
    fprintf('Iterations = %d;  Cost = %.4f; u = %.4f', iter, cost, u);
end
...
cost_run = zeros(1,Nu);
for i=1:Nu
    %cost_run_wt = l(x(:,i),u(:,i),t(i));
    %cost_run(i) = cost_run_wt * dt;
    [tmp, ~, ~ ,~, ~, ~] = l(x(:,i),u(:,i),t(i));
    cost_run(i) = tmp*dt;
end
